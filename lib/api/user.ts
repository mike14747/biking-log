...
export async function editRideData(dataId: number, date: Date, distance: number, timeDuration: string | undefined, avgSpeed: number | undefined, temperature: string, windSpeed: string, windDir: string, location: string, notes: string | undefined) {
    if (!dataId || !date || !distance) return null;

    const queryString = 'UPDATE data SET date=?, distance=?, time_duration=?, avg_speed=?, temperature=?, wind_speed=?, wind_dir=?, location=?, notes=?) WHERE id=? LIMIT 1';
    const queryParams = [
        date,
        distance,
        timeDuration || '',
...
export async function deleteRideData(dataId: number) {
    if (!dataId) return null;

    const queryString = 'DELETE FROM data WHERE id=?';
    const queryParams = [dataId];
    const result = await runQuery(queryString, queryParams);
